...
$('#wedding-title').text('The Wedding');
$('#couple-names').text(coupleNames);
$('#wedding-date').text('31 Desember 2023');

$('#title').text('The Wedding');
$('#image-container').css('background-image', 'url("assets/img/profil.jpg")');
$('#overlay-image').attr('src', 'assets/img/bg2.png');
$('#couple-names-home').text(coupleNames);
$('#day-text').text('- Minggu -');
$('#date-text').text('31 Desember 2023');
...
const coupleData = {
    ...
    invitation: "Tanpa mengurangi rasa hormat, kami mengundang Bapak/Ibu dan Saudara/i menghadiri acara Pernikahan kami",
    bride: {
        name: "Dewi Nurhaliza Putri",
        parent: "Putri dari Bapak Budi & Ibu Fitri Ayu",
        socials: [
    ...
    groom: {
        ...
        parent: "Putra dari Bapak Suryadi & Ibu Yuliana",
        socials: [
            { platform: "facebook", url: "#" },
            { platform: "instagram", url: "#" },
            { platform: "twitter", url: "#" }
...
$('#invitation').text(coupleData.invitation);
$('#bride-name').text(coupleData.bride.name);
$('#bride-parent').text(coupleData.bride.parent);
$('#groom-name').text(coupleData.groom.name);
$('#groom-parent').text(coupleData.groom.parent);

coupleData.bride.socials.forEach(social => {
    $('#bride-socials').append(`<a href="${social.url}" class="m-t-15" target="_blank"><i class="fab fa-${social.platform}-square"></i></a>`);
});

coupleData.groom.socials.forEach(social => {
    $('#groom-socials').append(`<a href="${social.url}" class="m-t-15" target="_blank"><i class="fab fa-${social.platform}-square"></i></a>`);
});
// ========================= END: 2. Couple Data ==========================
...
const eventData = {
    ...
    akad: {
        ...
        mapLink: "https://www.google.com/maps/dir/?api=1&amp;destination=Masjid+Al-Aqsha"
    },
    resepsi: {
        title: "Resepsi",
        day: "Minggu",
...
$('#save-the-date-title').text(eventData.saveTheDate);
...
$('#resepsi-title').text(eventData.resepsi.title);
$('#resepsi-day').text(eventData.resepsi.day);
$('#resepsi-date').text(eventData.resepsi.date);
$('#resepsi-time').text(eventData.resepsi.time);
$('#resepsi-location').text(eventData.resepsi.location);
$('#resepsi-map-link').attr('href', eventData.resepsi.mapLink);
// ========================= END: 4. Event Data ==========================
// ========================= 5. Gallery =========================
const galleryImages = [
...
galleryImages.forEach(image => {
    const galleryItem = `
        <a href="${image.src}" class="col-md-4 col-sm-6 aos-item" data-lg-size="1600-2400" data-sub-html="${image.subtitle}" data-aos="zoom-in" data-aos-duration="1000">
...
// ========================= 9. Penutup =========================
$('#profileImage').attr('src', 'assets/img/profil.jpg');
$('#coupleNames').text('Dewi & Dary');
